...
for row in results:
	...
	shipid = int(row[2])
	shipClass = int(row[3])
	fountainRequest = urllib2.Request('http://api.eve-central.com/api/marketstat?sethours=28800&regionlimit=10000060&typeid='+str(shipid))
	...
	print "--Delve buy:  volume: "+str(fountainBuyVolume)+" min: "+str(fountainBuyMin)+" max: "+str(fountainBuyMax)
	print "--Delve sell: volume: "+str(fountainSellVolume)+" min: "+str(fountainSellMin)+" max: "+str(fountainSellMax)
